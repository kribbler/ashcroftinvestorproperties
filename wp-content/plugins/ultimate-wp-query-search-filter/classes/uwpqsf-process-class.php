...
if(!class_exists('uwpqsfprocess')){
  ...
  function uwpqsf_ajax(){
	...
	if(isset($formid) && wp_verify_nonce($nonce, 'uwpsfsearch')){
  	...
  	/** bellow function made obsolete by daniel and replaced with a new one 
	function uajax_result($arg, $id,$pagenumber,$getdata){
		...
		if ( $query->have_posts() ) {
			...
			while ( $query->have_posts() ) {
				...
				$html .= 'x<article><header class="entry-header">'.get_the_post_thumbnail().'';
	...
	*/
	...
	/** bellow function altered by daniel **/
	...
	function uajax_result($arg, $id,$pagenumber,$getdata){
		...
		if ( $query->have_posts() ) {
			...
			$html .= '<h3>'.__('Search Results :', 'UWPQSF' ).'</h3>';
			while ( $query->have_posts() ) {
	        	$query->the_post();global $post;
	        	$count = ( $count == $columns ) ? 1 : $count + 1;
				$first_class = ( 1 == $count ) ? 'first' : '';

	        	$html .= '<div class="listing-wrap ' . get_column_class($columns) . ' ' . $first_class . '"><div class="listing-widget-thumb"><a href="' . get_permalink() . '" class="listing-image-link">' . get_the_post_thumbnail( $post->ID, 'listings' ) . '</a>';
	        	$html .= '<div class="listing-thumb-meta">';
	        	if ( '' != get_post_meta( $post->ID, '_listing_price', true ) ) {
		            $html .= 'x<span class="listing-price">' . number_format( get_post_meta( $post->ID, '_listing_price', true )) . '</span>';
		        }
		        $html .= '</div><!-- .listing-thumb-meta --></div><!-- .listing-widget-thumb -->';
		        if ( '' != get_post_meta( $post->ID, '_listing_open_house', true ) ) {
		            $html .= '<span class="listing-open-house">Open House: ' . get_post_meta( $post->ID, '_listing_open_house', true ) . '</span>';
		        }

		        $html .= '<div class="listing-widget-details">';
		        $html .= '<h3 class="listing-title">The <a href="' . get_permalink() . '">' . get_the_title() . ' <span class="the_location">' . strip_tags(get_the_term_list( $post->ID, "locations")) . '</span></a></h3>';
		        $html .= '<span class="open_detail" id="open_detail_'.$post->ID.'">+</span>';

		        $html .= '<div id="lis_des_'.$post->ID.'" class="hidden_listing_description">';
		            $html .= '<ul class="listing-meta">';
		                $html .= '<li class="listing-bedrooms">';
		                    $html .= get_post_meta( $post->ID, '_listing_bedrooms', true ); 
		                $html .= '</li>';
		                $html .= '<li class="listing-bathrooms">';
		                    $html .= get_post_meta( $post->ID, '_listing_bathrooms', true );
		                $html .= '</li>';
		                $html .= '<li class="listing-livingrooms">';
		                    $html .= get_post_meta( $post->ID, '_listing_livingrooms', true );
		                $html .= '</li>';
		                $html .= '<li class="listing-garages">';
		                    $html .= get_post_meta( $post->ID, '_listing_garages', true );
		                $html .= '</li>';
		                $html .= '<li class="listing-sqft">';
		                    $html .= '<span class="whiteish_sqft">FLOOR AREA</span> ' . get_post_meta( $post->ID, '_listing_sqft', true );
		                $html .= '</li>';
		            $html .= '</ul>';
		            ...
		            $html .= '<a class="listings_view_plans" href="' . get_permalink( $post->ID ) . '">view the ' . $post->post_title . ' plans</a>';
		        $html .= '</div>';     //div hidden_listing_description
		        $html .= '<div class="clearfix"></div>';
		        $html .= '</div><!-- .listing-widget-details --></div><!-- .listing-wrap -->';

				//$html .= 'y<article><header class="entry-header">'.get_the_post_thumbnail().'';
				//$html .= '<h1 class="entry-title"><a href="'.get_permalink().'" rel="bookmark">'.get_the_title().'</a></h1>';
				//$html .= '</header>';
				//$html .= '<div class="entry-summary">'.get_the_excerpt().'</div></article>';
			}

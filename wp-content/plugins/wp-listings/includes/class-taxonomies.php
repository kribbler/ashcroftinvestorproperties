...
class WP_Listings_Taxonomies {
	...
	/** daniel's code to create plan name taxonomy **/
	/**
	 * Register the plan-names taxonomy, manually.
	 */
	function plan_name_taxonomy() {

		$name = __( 'Plan Names', 'wp_listings' );
		$singular_name = __( 'Plan Name', 'wp_listings' );
		...
		return array(
			'plan-names' => array(
				...
				'hierarchical' => true,
				'rewrite' => array( __( 'plan-names', 'wp_listings' ) ),
				'editable' => 0
			)
		);
	...
	function get_taxonomies() {
		...
		return array_merge( 
			$this->listing_status_taxonomy(), 
			$this->listing_location_taxonomy(), 
			$this->property_type_taxonomy(), 
			$this->plan_name_taxonomy(), //daniel's code
			$this->property_features_taxonomy(), (array) get_option( $this->settings_field ) );
	...
	function create_terms() {
		...
		/** Daniel's code Default terms for plan-name */
		$plan_name_terms = apply_filters( 'wp_listings_default_plan_name_terms', array('Rocheford' => 'rocheford', 'Stamford' => 'stamford', 'Ascott' => 'ascott', 'Augusta' => 'augusta', 'Beaumont' => 'beaumont', 'Radfield' => 'radfield', 'Lyndhurst' => 'lyndhurst' ) );
		foreach ($plan_name_terms as $term => $slug) {
			if (term_exists($term, 'plan-names')) {
				continue;
			}
			wp_insert_term($term,'plan-names', array('slug' => $slug));
		}
